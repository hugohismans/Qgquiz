...
if (typeof document !== "undefined") (function () {
  ...
  /* Le son ne peut pas démarrer avant qu'on ait touché la page — tous les
     ...
     cliquer pour jouer. */
  function eveillerLeSon() {
    SON.reveiller();
    if (!SON.enSourdine()) SON.ambiance(true);
    document.removeEventListener("pointerdown", eveillerLeSon);
    document.removeEventListener("keydown", eveillerLeSon);
  }
  document.addEventListener("pointerdown", eveillerLeSon);
  document.addEventListener("keydown", eveillerLeSon);

  function peindreBoutonSon() {
    const b = $("b-son");
    const muet = SON.enSourdine();
    b.setAttribute("aria-pressed", muet ? "false" : "true");
    b.textContent = muet ? "\u266A\u0338" : "\u266A";
    b.setAttribute("aria-label", muet ? "Remettre le son" : "Couper le son");
  }
  $("b-son").onclick = function () {
    SON.reveiller();
    SON.basculerSourdine();
    peindreBoutonSon();
  };
  peindreBoutonSon();
  ...
  /* Les crédits. Les photos viennent de Wikimedia Commons sous licence
     Creative Commons : nommer l'auteur est la condition d'usage, pas une
     politesse qu'on rendrait si on y pense. */
  function peindreCredits() {
    const zone = $("credits");
    if (typeof CREDITS === "undefined" || !CREDITS.length) {
      $("b-credits").style.display = "none";
      return;
    }
    let html = "<p>Les photos viennent de Wikimedia Commons. Chacune reste à son "
             + "auteur, sous la licence indiquée.</p><ul>";
    CREDITS.forEach(function (c) {
      html += "<li><b>" + c.sujet + "</b>"
            + (c.lieu ? c.lieu + " · " : "")
            + c.auteur + " · " + c.licence
            + ' · <a href="' + c.lien + '" target="_blank" rel="noopener">la page d\'origine</a>'
            + "</li>";
    });
    zone.innerHTML = html + "</ul>";
  }
  peindreCredits();
  $("b-credits").onclick = function () {
    const zone = $("credits");
